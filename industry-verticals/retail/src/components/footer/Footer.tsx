...
export const Default = (props: FooterProps) => {
  ...
  return (
    <section className={`component footer relative ${props.params.styles} overflow-hidden`} id={id}>
      <div className="bg-dfin-navy text-white">
        <div className="container grid gap-12 py-20 lg:grid-cols-[1fr_3fr]">
          <div className="flex flex-col gap-6">
            <div className="brightness-0 invert sm:max-w-40">
              <Image field={props.fields.Logo} />
            </div>
            <div className="text-white/70 [&_*]:text-inherit">
          ...
          <div className="grid gap-10 sm:grid-cols-3 lg:grid-cols-5 lg:gap-5 xl:gap-12">
            {sections.map(({ key, title, content }) => (
              <div key={key}>
                <div className="mb-6 text-lg font-bold text-white">{title}</div>
                <div className="[&_a:hover]:text-accent space-y-3 text-white/70 [&_a]:text-white/70 [&_a]:transition-colors">
                  {content}
                </div>
              </div>
            ))}
      ...
      <div className="bg-dfin-navy-light text-white/60">
        <div className="container flex items-center justify-between py-6 text-sm max-sm:flex-col max-sm:items-start max-sm:gap-6">
          ...
          <div className="flex items-center justify-between gap-12 max-sm:order-1 max-sm:flex-col max-sm:items-start max-sm:gap-3">
            <Link field={props.fields.TermsText} className="transition-colors hover:text-white" />
            <Link field={props.fields.PolicyText} className="transition-colors hover:text-white" />
          </div>
        </div>
      </div>
    </section>
